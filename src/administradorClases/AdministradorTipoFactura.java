...
public class AdministradorTipoFactura extends ConectorMySql {      
// atributos y variables  
    ...
    public AdministradorTipoFactura() {
...
// se pasa el valor de orden y acomoda los datos por ese tipo        
    public String ordenarTabla(String valor){
        ...
        switch(valor){
            ...
            case "Seleccionar":
                _resultFiltrar = "iuTipoFactura ASC";
            break; 
            ...
            case "Alfabeticos":
                _resultFiltrar = " descrTipoFactura ASC";
            break;
        }
...
// se genera la coneccion y el query de consulta 
        ...
        _mySqlSentencia = "SELECT iuTipoFactura, descrTipoFactura, estadoEliminacion FROM TipoFactura ORDER BY " + _resultFiltrar + "";
...
// se obtiene los datos de la db y se carga el modelo   
        ...
        while(rsMySql.next()){
            if(rsMySql.getInt("estadoEliminacion") == 1){
                ...
                registros[0] = rsMySql.getString("iuTipoFactura");
                registros[1] = rsMySql.getString("descrTipoFactura");  
...
// se genera la coneccion y el query de consulta        
        _conectarSql = getConection();   
        _mySqlSentencia = "SELECT iuTipoFactura, descrTipoFactura, estadoEliminacion FROM TipoFactura WHERE iuTipoFactura LIKE '%"+ busqueda+"%' OR descrTipoFactura LIKE '%"+ busqueda+"%' ";
...
// se inserta datos en la db  
    public boolean insertarTablaTiposFacturas(TipoFactura obj){
        boolean verdad = false;
        int resultadoInsercion;
// se genera la coneccion y el query de consulta        
        _conectarSql = getConection();
        _mySqlSentencia = "INSERT INTO TipoFactura( descrTipoFactura) VALUES( ?)";
    try {
// se prepara la insercion y se inyecta los datos
        ...
        _mySqlPreparaSentencia.setString(1, obj.getDescrTipoFactura());
...
// se modifican los datos  en la db  
    public boolean actualizarTablaTiposFactutas(TipoFactura obj){
        boolean verdad = false;
        int resultadoActualizacion;
// se genera la coneccion y el query de consulta        
        _conectarSql = getConection(); 
        _mySqlSentencia = "UPDATE TipoFactura SET   descrTipoFactura = ?  WHERE iuTipoFactura = ?";
    try {
...
// eliminan los datos del sistema no de la db
    public boolean eliminarTablaTiposFacturas(TipoFactura obj){
        boolean verdad = false;
        int resultadosEliminacion;
// se genera la coneccion y el query de consulta        
        _conectarSql = getConection();
        _mySqlSentencia = "UPDATE  TipoFactura SET estadoEliminacion = ? WHERE iuTipoFactura = ?";
    try {
// se prepara la actualizacion y se inyecta los datos nuevos   
        PreparedStatement _mySqlPreparaSentencia = _conectarSql.prepareStatement(_mySqlSentencia);
...
// este metodo busca lo ingresado y muetra el resultado    
    public String obtenerIuTipoFactura(String busqueda){
        String aux = "";
// se genera la coneccion y el query de consulta        
        _conectarSql = getConection();
        _mySqlSentencia = "SELECT iuTipoFactura, descrTipoFactura FROM tipofactura WHERE  descrTipoFactura = '"+ busqueda+"' OR iuTipoFactura = '"+busqueda+"'  ";
    try {
// se obtiene los datos de la db y se carga el modelo  
        Statement stMySql = (Statement) _conectarSql.createStatement(); 
        ResultSet rsMySql = stMySql.executeQuery(_mySqlSentencia);
        while(rsMySql.next()){
                ...
                aux = rsMySql.getString("iuTipoFactura");
        }
...
// se cierra la conexion con la db
    ...
    // este metodo busca lo ingresado y muetra el resultado    
    public String obtenerDescrTipoFactura(String busqueda){
        String aux = "";
// se genera la coneccion y el query de consulta        
        _conectarSql = getConection();
        _mySqlSentencia = "SELECT iuTipoFactura, descrTipoFactura FROM tipofactura WHERE  descrTipoFactura = '"+ busqueda+"' OR iuTipoFactura = '"+busqueda+"'  ";
...
// se obtiene los datos de la db y se carga el modelo  
        ...
        while(rsMySql.next()){
                ...
                aux = rsMySql.getString("descrTipoFactura");
        }
...
// se genera la coneccion y el query de consulta        
        ...
        _conectarSql = getConection();
        _mySqlSentencia = "SELECT iuTipoFactura, descrTipoFactura, estadoEliminacion FROM tipoFactura WHERE iuTipoFactura LIKE '%"+ busqueda+"%' OR descrTipoFactura LIKE '%"+ busqueda+"%' ";
        _mySqlSentencia2 = "SELECT iuTipoFactura, descrTipoFactura, estadoEliminacion FROM tipoFactura ORDER BY " + _resultFiltrar + "";
    try {
// se obtiene los datos de la db y se carga el modelo  
        if(busqueda.length()!=0){
            ...
            while(rsMySql.next()){
                if(rsMySql.getInt("estadoEliminacion") == 1 ){
                    ...
                    registros.add(rsMySql.getString("descrTipoFactura")); 
                }    
            }
        } else{
            Statement stMySql = (Statement) _conectarSql.createStatement(); 
